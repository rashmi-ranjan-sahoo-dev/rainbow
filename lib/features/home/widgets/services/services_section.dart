...
/// Data model for a Clinical Symptom Item.
class _SymptomData {
  final FaIconData icon;
  final String title;
  final String description;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // ── 3. Symptoms & Disease Assessment Banner ──
          Padding(
            padding: EdgeInsets.symmetric(
              horizontal: ResponsiveHelper.horizontalPadding(context),
            ),
            child: ConstrainedBox(
              constraints: const BoxConstraints(maxWidth: 1320),
              child: const _SymptomsAwarenessBanner(),
            ),
          ),
        ],
      ),
...
/// Bottom Symptom Awareness Banner with animated radar pulse and interactive touch cards.
class _SymptomsAwarenessBanner extends StatefulWidget {
  const _SymptomsAwarenessBanner();

  @override
  State<_SymptomsAwarenessBanner> createState() => _SymptomsAwarenessBannerState();
}

class _SymptomsAwarenessBannerState extends State<_SymptomsAwarenessBanner>
    with SingleTickerProviderStateMixin {
  late final AnimationController _pulseController;
  int _activeSymptomIndex = 0;
  bool _isViewAllMobile = false;

  static const _symptoms = [
    _SymptomData(
      icon: FontAwesomeIcons.circleExclamation,
      title: 'Flashes or Dark Floaters?',
      description:
          'Potential Retinal tear or Diabetic retinopathy. Requires immediate same-day specialized retina checkup.',
      accentColor: Color(0xFFE11D48),
      badge: '⚡ Urgent Retina Diagnostic',
      isUrgent: true,
    ),
    _SymptomData(
      icon: FontAwesomeIcons.cloud,
      title: 'Cloudy / Foggy Vision?',
      description:
          'Likely Cataract development. Our 10-minute micro-incision phaco surgery restores 100% crystal clarity.',
      accentColor: AppColors.primary,
      badge: 'Age 50+ Common',
    ),
    _SymptomData(
      icon: FontAwesomeIcons.glasses,
      title: 'Tired of Daily Spectacles?',
      description:
          'Contoura Vision & SMILE lasers correct nearsightedness, farsightedness & astigmatism permanently.',
      accentColor: AppColors.accent,
      badge: '👓 100% Spectacle Free',
    ),
    _SymptomData(
      icon: FontAwesomeIcons.fire,
      title: 'Acute Pain, Redness & Halos?',
      description:
          'Potential Acute Angle Glaucoma or Corneal Abrasion. Requires immediate 24x7 emergency pressure check.',
      accentColor: Color(0xFFD97706),
      badge: '🔴 24x7 Urgent Check',
      isUrgent: true,
    ),
  ];
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          // Header Bar with Pulsing Radar Beacon
          Row(
            crossAxisAlignment: CrossAxisAlignment.center,
            children: [
              Stack(
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      'Find your clinical department in seconds based on what you feel.',
                      style: TextStyle(
                        fontFamily: 'Inter',
          ...
          // Symptoms Grid / Selector with Slow Smooth Animated Expansion
          LayoutBuilder(
            builder: (context, constraints) {
              if (isMobile) {
                return Column(
                  children: [
                    // Always show the first 2 primary symptoms
                    for (int i = 0; i < 2; i++) ...[
                      _SymptomCard(
                        data: _symptoms[i],
                        isSelected: _activeSymptomIndex == i,
                        onTap: () {
                          setState(() => _activeSymptomIndex = i);
                          showBookingDialog(context);
                        },
                      ),
                    ...
                    // Smoothly & slowly animate the 3rd and 4th symptoms open / close
                    AnimatedSize(
                      duration: const Duration(milliseconds: 650),
                      reverseDuration: const Duration(milliseconds: 600),
                      curve: Curves.easeInOutCubic,
                      alignment: Alignment.topCenter,
                      child: _isViewAllMobile
                          ? Padding(
                              padding: const EdgeInsets.only(top: 12),
                              child: Column(
                                children: [
                                  for (int i = 2; i < _symptoms.length; i++) ...[
                                    _SymptomCard(
                                      data: _symptoms[i],
                                      isSelected: _activeSymptomIndex == i,
                                      onTap: () {
                    ...
                    TextButton.icon(
                      onPressed: () {
                        setState(() => _isViewAllMobile = !_isViewAllMobile);
                      },
                      icon: AnimatedRotation(
                        duration: const Duration(milliseconds: 450),
                        curve: Curves.easeInOutCubic,
                        turns: _isViewAllMobile ? 0.5 : 0.0,
                        child: const Icon(
                      ...
                      label: Text(
                        _isViewAllMobile ? 'Show Less Symptoms' : 'View All 4 Symptoms',
                        style: const TextStyle(
                          fontFamily: 'Poppins',
                          fontSize: 13,
                          fontWeight: FontWeight.w600,
                  ...
                  return Expanded(
                    child: Padding(
                      padding: EdgeInsets.only(
                        right: index < _symptoms.length - 1 ? 12 : 0,
                      ),
                      child: _SymptomCard(
                        data: item,
                        isSelected: _activeSymptomIndex == index,
                        onTap: () {
                          setState(() => _activeSymptomIndex = index);
                          showBookingDialog(context);
                        },
                      ),
                    ),
    ...
    return MouseRegion(
      ...
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            color: isHighlighted
                ? d.accentColor.withValues(alpha: 0.05)
                : const Color(0xFFF8FAFC),
            borderRadius: BorderRadius.circular(16),
            border: Border.all(
              color: isHighlighted
                  ? d.accentColor.withValues(alpha: 0.45)
                  : const Color(0xFFE2E8F0),
                ...
                children: [
                  ...
                  Flexible(
                    child: Container(
                      padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                      decoration: BoxDecoration(
                        color: d.accentColor.withValues(alpha: 0.10),
                        borderRadius: BorderRadius.circular(4),
                      ),
                      child: Text(
                        d.badge,
